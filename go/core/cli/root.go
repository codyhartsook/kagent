// Package cli constructs the kagent CLI command tree.
package cli
...
func New(cfg *config.Config) *cobra.Command {
	...
	installCfg := &agentcli.InstallCfg{
		Config: cfg,
	}
	...
	installCmd := &cobra.Command{
		...
		Long:  `Install kagent`,
		Run: func(cmd *cobra.Command, args []string) {
			agentcli.InstallCmd(cmd.Context(), installCfg)
		},
	}
	...
	uninstallCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			agentcli.UninstallCmd(cmd.Context(), cfg)
		},
	}
	...
	bugReportCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			if err := agentcli.CheckServerConnection(cmd.Context(), cfg.Client()); err != nil {
				pf, err := agentcli.NewPortForward(cmd.Context(), cfg)
				if err != nil {
	...
	versionCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			defer agentcli.VersionCmd(cfg)

			if err := agentcli.CheckServerConnection(cmd.Context(), cfg.Client()); err != nil {
				if pf, e := agentcli.NewPortForward(cmd.Context(), cfg); e == nil {
					defer pf.Stop()
				}
			}
		},
	...
	dashboardCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			agentcli.DashboardCmd(cmd.Context(), cfg)
		},
	}
	...
	getSessionCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			agentcli.GetSessionCmd(cfg, resourceName)
		},
	}
	...
	getAgentCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			agentcli.GetAgentCmd(cfg, resourceName)
		},
	}
...
Examples:
  ...
  kagent init adk python dice --model-provider Gemini --model-name gemini-2.5-flash`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err := agentcli.InitCmd(initCfg); err != nil {
...
Examples:
  ...
  kagent build ./my-agent --image ghcr.io/myorg/my-agent:v1.0.0 --push`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err := agentcli.BuildCmd(buildCfg); err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		},
...
Examples:
  ...
  kagent deploy ./my-agent --env-file .env --dry-run > manifests.yaml`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			var err error
			if !deployCfg.DryRun {
				k8sClient, err = agentcli.CreateKubernetesClient()
				if err != nil {
			...
			if err := agentcli.DeployCmd(cmd.Context(), k8sClient, deployCfg); err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		},
	...
	// add-mcp command
	addMcpCfg := &agentcli.AddMcpCfg{Config: cfg}
	addMcpCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err := agentcli.AddMcpCmd(addMcpCfg); err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		},
...
Examples:
  ...
  kagent run .`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err := agentcli.RunCmd(cmd.Context(), runCfg); err != nil {
...
func runInteractive(cmd *cobra.Command, args []string, cfg *config.Config) {
	...
	var pf *agentcli.PortForward
	if err := agentcli.CheckServerConnection(cmd.Context(), client); err != nil {
		pf, err = agentcli.NewPortForward(cmd.Context(), cfg)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error starting port-forward: %v\n", err)
			return
